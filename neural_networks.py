...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['W'] = np.dot(X.T, grad)                #du/dw = x.T . grad 
        self.gradient['b'] = np.sum(grad, axis=0)             #du/db = 1 for some b
        

        backward_output = np.dot(grad, self.params['W'].T)     #du/dx = w.T . grad
...
class relu:
    ...
    """
        self.mask can be used to store things (computed in the forward pass) for the use in the backward pass.
    """
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        """
            ...
            - backward_output: A numpy array of the same shape as X, where each element is the partial derivative of the mini-batch loss w.r.t. the corresponding element in  X.
        """
        backward_output = (1 - np.square(np.tanh(X))) * grad
